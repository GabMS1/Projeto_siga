...
// Inicializa as variáveis do dashboard
$totalDisciplinas = 0;
$totalTurmas = 0;
$reposicoesDisponiveis = 0; // Renomeado para clareza
$totalCargaHoraria = 0;
$horasRestantes = 0;
$reposicoesAgendadas = [];
...
if ($conn) {
    // 1. Pega os totais de disciplinas, carga horária e aulas semanais
    ...
    // 3. Contar reposições disponíveis (sem substituto).
    ...
    // 4. Buscar as 3 próximas reposições agendadas.
...
<head>
    ...
    <style>
        ...
        body { margin: 0; font-family: 'Poppins', sans-serif; display: flex; background-color: var(--bg-light); color: var(--text-color); min-height: 100vh; }
        .sidebar { width: 250px; background-color: var(--primary-color); color: white; padding: 20px 0; position: fixed; height: 100%; box-shadow: var(--shadow); display: flex; flex-direction: column; align-items: center; }
        .sidebar h2 { font-size: 1.5em; font-weight: 700; margin-bottom: 30px; }
        ...
        .header h1 { color: var(--text-color); font-size: 1.8em; font-weight: 600; margin: 0; }
        .cards { display: grid; grid-template-columns: repeat(auto-fit, minmax(240px, 1fr)); gap: 25px; }
        .card { background-color: var(--card-bg); padding: 25px; border-radius: 12px; box-shadow: var(--shadow); display: flex; align-items: center; gap: 20px; transition: transform 0.3s ease, box-shadow 0.3s ease; }
        .card:hover { transform: translateY(-5px); box-shadow: 0 6px 20px rgba(0, 0, 0, 0.12); }
        .card-icon { font-size: 2.2em; padding: 15px; border-radius: 50%; color: white; }
        .card-icon.disciplinas { background-color: #2a9d8f; }
        .card-icon.carga-horaria { background-color: #e76f51; }
        .card-icon.turmas { background-color: #f4a261; }
        .card-icon.faltas { background-color: #e63946; }
        .card-icon.horas-restantes { background-color: #457b9d; }
        .card-info h3 { margin: 0; color: #888; font-size: 0.9em; font-weight: 500; text-transform: uppercase; }
        .card-info p { font-size: 1.8em; font-weight: 700; color: var(--text-color); margin: 5px 0 0 0; }
        .section { margin-top: 40px; background-color: var(--card-bg); padding: 30px; border-radius: 12px; box-shadow: var(--shadow); }
        .section h2 { margin-top: 0; margin-bottom: 20px; color: var(--text-color); border-bottom: 1px solid var(--border-color); padding-bottom: 10px; font-size: 1.3em; font-weight: 600; }
        .list-item { display: flex; align-items: center; gap: 15px; padding: 15px; border-radius: 8px; margin-bottom: 10px; border: 1px solid var(--border-color); }
        .list-item i { color: var(--secondary-color); font-size: 1.2em; }
...
<div class="main-content">
    <div class="header">
        <h1>Bem-vindo(a), <?php echo htmlspecialchars(explode(' ', $nome)[0]); ?>!</h1>
    </div>

    <div class="cards">
        <div class="card">
            <div class="card-icon carga-horaria"><i class="fas fa-hourglass-start"></i></div>
            <div class="card-info">
                <h3>Carga Total</h3>
        ...
        <div class="card">
            <div class="card-icon horas-restantes"><i class="fas fa-hourglass-half"></i></div>
            <div class="card-info">
        ...
        <div class="card">
            <div class="card-icon disciplinas"><i class="fas fa-book-open"></i></div>
            <div class="card-info">
                <h3>Disciplinas</h3>
        ...
        <div class="card">
            <div class="card-icon turmas"><i class="fas fa-chalkboard-teacher"></i></div>
            <div class="card-info">
                <h3>Turmas</h3>
        ...
        <a href="calendario.php" class="card-link">
            <div class="card">
                <div class="card-icon faltas"><i class="fas fa-hand-holding-hand"></i></div>
                <div class="card-info">
        ...
        if (!empty($reposicoesAgendadas)) {
            foreach ($reposicoesAgendadas as $row) {
                echo "<div class='list-item'>
                        ...
                        <small>" . htmlspecialchars($row['nome_disciplina']) . " - " . htmlspecialchars($row['curso'] . " " . $row['serie']) . "</small></div>
                      </div>";
            }
        } else {
